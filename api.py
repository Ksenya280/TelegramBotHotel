...
def get_hotels(request: Request, city: str = None, sort_order: str = None,
    ...
    response = requests.request("POST", 'https://hotels4.p.rapidapi.com/properties/v2/list', headers=headers,
                                json=querystring)
    ...
    for hotel in data['data']['propertySearch']['properties']:
        ...
        if len(godCity.distance):
            if (int(godCity.distance[1]) >= distance >= int(godCity.distance[0])) and (
                    int(godCity.range_prices[1]) >= int(price_without_dollar) >= int(godCity.range_prices[0])):
                godCity.all_hotels = {"name": name, "stars": stars, "url_photo": url_photo, "hotel_id": id,
                                      "price": price, "distance": distance}
        else:
            godCity.all_hotels = {"name": name, "stars": stars, "url_photo": url_photo, "hotel_id": id, "price": price,
                                  "distance": distance}
        num += 1
